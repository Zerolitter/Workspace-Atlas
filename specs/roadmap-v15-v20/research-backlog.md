...
- **Research question:** Can TypeSafe JEV System One judgments predict the minimum useful `DIRECT | ATLAS_LIGHT | ATLAS_DEEP` acquisition depth, escalation need, and selected task-policy flags better than the current deterministic policy without weakening correctness or Atlas invariants?
- **Official integration shape:** use the TypeSafe System One programming model and official SDK: Choice for route/task-kind selection, Noul for escalation/history/validation probabilities, and Score for graded project-context risk. Keep independent questions over the same state in one evaluation and preserve raw probability outputs.
- **Authorized preparation:** an external research adapter, bounded task/metadata state, typed question pack, dry-run inspection, explicit opt-in live call through the official SDK, redacted/hash-based local output, and comparison against existing Atlas route/Context Yield evidence.
- **Not authorized:** modifying `atlas governor run`; giving JEV Truth Plane, Serving, source-verification, Context IR, lifecycle, catalogue, MCP, mutation, or destructive authority; adding a production dependency or catalogue migration; transmitting raw source by default; treating Choice/Score confidence, Noul probabilities, or any prediction as project evidence.
- **Preserved invariants:** deterministic Atlas routing remains authoritative; source stays authoritative; prediction never becomes evidence; exact-source live verification remains mandatory; Atlas works fully offline with no TypeSafe/JEV configuration.
...
- **Evaluation:** compare JEV route distributions and typed judgments to the route Atlas actually required, later escalation, supplied/used working sets, context expansion, local work/latency, accepted outcome, TypeSafe token usage, and Context Yield where valid.
